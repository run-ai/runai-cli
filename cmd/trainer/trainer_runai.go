...
func (rt *RunaiTrainer) IsSupported(name, ns string) bool {
	...
	pod, err := rt.client.CoreV1().Pods(ns).Get(context.TODO(), name, metav1.GetOptions{})
...
func (rt *RunaiTrainer) GetTrainingJob(name, namespace string) (TrainingJob, error) {
	...
	pod, err := rt.client.CoreV1().Pods(namespace).Get(context.TODO(), name, metav1.GetOptions{})
...
func (rt *RunaiTrainer) getRunaiTrainingJob(podSpecJob cmdTypes.PodTemplateJob, namespace string) (TrainingJob, error) {
	...
	return NewRunaiWorkload(filteredPods, lastCreatedPod, podSpecJob.CreationTimestamp, jobType, podSpecJob.Name, podSpecJob.Labels["app"] == "runaijob", serviceUrls, false, podSpecJob.Template.Spec, podSpecJob.Template.ObjectMeta, podSpecJob.ObjectMeta, podSpecJob.Namespace, ownerResource, status, podSpecJob.Parallelism, podSpecJob.Completions, podSpecJob.Failed, podSpecJob.Succeeded), nil
}
...
func (rt *RunaiTrainer) getPodsOfJob(podSpecJob cmdTypes.PodTemplateJob) []v1.Pod {
	...
	podList, err := rt.client.CoreV1().Pods(podSpecJob.Namespace).List(context.TODO(), metav1.ListOptions{
	...
	if podSpecJob.Type == cmdTypes.ResourceTypeDeployment {
		rsList, err := rt.client.AppsV1().ReplicaSets(podSpecJob.Namespace).List(context.TODO(), metav1.ListOptions{
			TypeMeta: metav1.TypeMeta{
				Kind:       "ListOptions",
				APIVersion: "v1",
			},
	...
	if len(filteredPods) == 0 {
		pod, err := rt.client.CoreV1().Pods(podSpecJob.Namespace).Get(context.TODO(), podSpecJob.Name, metav1.GetOptions{})
...
func (rt *RunaiTrainer) ListTrainingJobs(namespace string) ([]TrainingJob, error) {
	...
	for _, jobInfo := range jobPodMap {
		...
		runaiJobs = append(runaiJobs, NewRunaiWorkload(jobInfo.pods, lastCreatedPod, jobInfo.creationTimestamp, jobInfo.jobType, jobInfo.name, jobInfo.createdByCLI, serviceUrls, jobInfo.deleted, jobInfo.podSpec, jobInfo.podMetadata, jobInfo.ObjectMeta, jobInfo.namespace, jobInfo.owner, jobInfo.status, jobInfo.parallelism, jobInfo.completions, jobInfo.failed, jobInfo.succeeded))
	}
...
func (rt *RunaiTrainer) getPodJobMap(namespace string) (map[types.UID]*RunaiJobInfo, error) {
	...
	replicaSets, err := rt.client.AppsV1().ReplicaSets(namespace).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
